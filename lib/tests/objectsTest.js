...
PlayerController.prototype.setMousePos = function setMousePos(x,y) {
	...
	if(calcDir != player.state.direction) {
		...
		force = (DIRS.none == player.state.direction ? player.physics.forces.newMove : player.physics.forces.newDirection);
		
		player.state.direction = calcDir;
		
		this.movementTick(force);
...
PlayerController.prototype.movementTick = function movementTick(force) {
	...
	if(null != player.physics.body.m_contactList) {
		this.applyDirection(force);
	}
}

/**
 * Adds the correct force for the  player direction
 */
PlayerController.prototype.applyDirection = function applyDirection(force) {
	var player = this.playerModel,
		newForce = this.getDirectionForce(player.state.direction, force);
